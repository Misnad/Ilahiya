...
def login():
    ...
    if request.method == 'POST':
        ...
        if user:
            ...
            flash('E-mail does not exit. Please create an account first.',
                  category='error')
...
def newuser():
    if request.method == 'POST':
        ...
        password = generate_password_hash(
            request.form['password'] + username, method='sha256')
        ...
        if len(name) < 4:
            ...
            flash("An account with the same email exists. Use login.",
                  category='error')
        else:
            flash("Account created!", category='success')
            db.session.add(Users(name=name, email=email,
                           username=username, salted_pass=password))
            db.session.commit()
